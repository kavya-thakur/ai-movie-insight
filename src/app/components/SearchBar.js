...
export default function SearchHeader({
  ...
  return (
    <motion.header
      // Removed generic 'layout' to stop the full-page re-render flicker
      className={`w-full max-w-7xl mx-auto px-6 md:px-12 relative z-10 transition-all duration-700 ease-[0.16,1,0.3,1] ${
    ...
    >
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-4">
          ...
          <motion.h1
            layout="position"
            // Use layout="position" so it slides smoothly without flickering the text color
            transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
            className={`font-light tracking-tighter leading-none ${
        ...
        <motion.div
          layout="position"
          transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          className={`relative flex items-center border-b-2 flex-shrink-0 ${
            error
              ? "border-red-500/50"
              : hasData
                ? "w-full md:w-1/2 border-foreground/10"
        ...
        >
          <input
            ...
            onKeyDown={(e) => e.key === "Enter" && onSearch()}
            placeholder="tt0133093"
            // Using placeholder:transition-none to prevent flickering on focus
            className="flex-1 min-w-[100px] bg-transparent py-4 text-2xl md:text-4xl font-light tracking-tight outline-none placeholder:text-foreground/10 placeholder:transition-none"
          />
